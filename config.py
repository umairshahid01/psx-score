...
ANNOUNCEMENTS = {
    ...
    "peek_max_pdfs": 4,        # how many PDFs may be opened per analyze
    "peek_max_pages": 2,       # pages read per PDF (verdict-relevant text is up front)
    "peek_max_mb": 8,          # skip PDFs bigger than this
    "peek_budget_s": 12,       # hard wall-clock cap for ALL pdf reads combined
}
...
# -----------------------------------------------------------------------------
RECOMMEND = {
    "pool_index": "KSE30",     # candidate pool (liquid, index-grade names)
    "extra_index": "KMI30",    # union'd in for wider coverage
    "max_symbols": 40,         # hard cap on the scan, politeness first
    "top_n": 5,                # picks surfaced on the dashboard
    "delay_s": 2.0,            # pause between symbols (be a good citizen)
