...
# v4.0 — MATERIAL INFORMATION section. Built from scratch: only filings whose
# title says "Material Information" are taken, their PDFs are read END TO END,
# and the verdict is EXTRACTED from the document's own words — never assumed.
MATERIAL_INFO = {
    "max_filings": 6,          # newest Material Information filings to read
    "max_pages": 8,            # read the WHOLE document (MI letters are short)
    "max_mb": 10,              # skip PDFs bigger than this
    "budget_s": 45,            # wall-clock cap for all reads combined
                               # (raised: CPU OCR of scanned letters needs room)
    "workers": 4,              # parallel PDF fetches
    "window_months": 12,       # how far back to look
    "cache_hours": 24,         # verdicts persist on disk — relaunches and the
                               # weekly rebuild never re-OCR the same letters
}
...
# -----------------------------------------------------------------------------
RECOMMEND = {
    "pool_index": "KSE100",    # THE pool — all three landing lists come strictly from here
    "max_symbols": 110,        # hard cap on the scan
    "top_n": 5,                # Top picks list size (max — fewer if fewer qualify)
    "avoid_n": 5,              # Stay-away list size (max)
    "blue_n": 5,               # Blue-chip list size (max)
    "top_min_score": 60,       # a Top pick must actually be strong
    "avoid_max_score": 48,     # a stay-away must actually be weak
    "blue_min_score": 60,      # a blue chip must be fundamentally solid
    "workers": 12,             # parallel evaluation threads
    ...
    "serve_stale": True,       # instantly serve last scan while rebuilding
...
ANALYSIS_TTL_MINUTES = 240    # serve a company's analysis from cache this long —
                              # the boot scan primes every KSE-100 company, so
                              # Fundamental / Technical clicks stay INSTANT for
                              # the whole session (fresh prices return whenever
                              # a new scan or relaunch refresh runs)
