...
def _warm_universe():
    ...
    # v4.0 speed rule — the prewarm now WAITS for the recommendations scan
    # to finish first, so the two never fight for bandwidth and the landing
    # lists land as fast as the network allows.
    def _prewarm_after_recommend():
        try:
            import recommend
            waited = 0
            while recommend.is_building() and waited < 1800:   # ≤30 min guard
                time.sleep(10)
                waited += 10
        except Exception:  # noqa: BLE001
            pass
