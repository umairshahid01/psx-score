...
def scrape_company(symbol: str, deep_pdf: bool = True) -> Dict:
    ...
    with ThreadPoolExecutor(max_workers=4, thread_name_prefix="scrape") as ex:
        ...
        utils.progress_update(symbol, 72,
                              "Announcements & catalysts scanned…")
    ...
    # ---- v3.6: Capital Adequacy ladder (banks & financials only) ----------
    sector_up = (profile.get("sector") or "").upper()
    if any(k in sector_up for k in config.FINANCIAL_SECTOR_KEYWORDS):
        utils.progress_update(symbol, 79, "Hunting the reported Capital Adequacy Ratio…")
        fill_capital_adequacy(symbol, session, page_html,
                              financials, sa_data, warnings)
