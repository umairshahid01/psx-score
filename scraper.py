...
def scrape_price_history(symbol: str, session) -> List[Dict]:
    ...
    v4.0 — two correctness fixes live here:

      1. TIMESTAMPS ARE READ IN PAKISTAN TIME. The old code used
         datetime.utcfromtimestamp(), which shifts a midnight-PKT session
         stamp back to 19:00 of the PREVIOUS day in UTC — silently mislabelling
         every bar in the series by one day. Every downstream date comparison
         (as-of checks, 52-week windows, weekly candle bucketing) inherited
         that error. utils.ts_to_psx_date() resolves the stamp in UTC+5.

      2. TODAY'S PARTIAL SESSION IS DROPPED. The as-of contract says all
         analysis describes the previous trading day's CLOSE. A bar dated today
         is an incomplete, still-moving session and must never enter a
    ...
    for row in rows:
        try:
            if isinstance(row, (list, tuple)) and len(row) >= 2:
                ...
                date = utils.ts_to_psx_date(ts) or str(ts)[:10]
    ...
    # v4.0 — enforce the as-of contract at the source. Nothing downstream ever
    # sees a bar newer than the previous trading day's close.
    trimmed, meta = utils.trim_history_to_as_of(cleaned)
    if meta.get("dropped"):
        print(f"  [asof] {symbol}: dropped {meta['dropped']} bar(s) dated after "
              f"{meta['as_of']} (today's partial session)")
    if meta.get("stale"):
        print(f"  [asof] {symbol}: WARNING feed ends {meta['last_close_date']}, "
              f"expected {meta['as_of']} ({meta['stale_days']} days behind)")
    return trimmed
...
def scrape_company(symbol: str, deep_pdf: bool = True) -> Dict:
    ...
    with ThreadPoolExecutor(max_workers=4, thread_name_prefix="scrape") as ex:
        ...
        # title-level classification but skips announcement PDF deep-reads.
        ...
        # ALWAYS runs — even in the ranking scan — so the analyses primed at
        # boot carry real MI verdicts and individual clicks are both instant
        # AND complete. A 24h disk cache in catalyst keeps the cost tiny.
    ...
    # ---- Price: the AS-OF CLOSE is authoritative -------------------------
    # v4.0 — this order was reversed before. A live intraday quote used to
    # overwrite the close, which meant the headline price, the chart, the
    # indicators and the trade plan were all describing different moments in
    # time. Everything the app computes must describe ONE session: the close
    # of the previous trading day. The live quote is still captured, but only
    # as a labelled reference field — it never feeds a calculation.
    live_price = _scrape_live_price(symbol, session) if page_html is None \
        else _price_from_html(symbol, page_html)
    if live_price and live_price > 0:
        profile["live_price"] = live_price
        profile["_live_price_as_of"] = scraped_at

    as_of_target = utils.as_of_iso()
    if price_history:
        last_pt = price_history[-1]
        profile["price"] = last_pt["close"]
        profile["_price_source"] = f"official close ({last_pt['date']})"
        profile["_price_as_of"] = last_pt["date"]
        profile["_as_of_target"] = as_of_target
        if last_pt["date"] < as_of_target:
            profile["_price_stale"] = True
            warnings.append(
                f"Latest official close available is {last_pt['date']}, but the "
                f"expected last trading session is {as_of_target}. Figures "
                f"describe {last_pt['date']}.")
    elif profile.get("price") and profile["price"] > 0:
        profile["_price_source"] = "company page (no EOD history available)"
        profile["_price_as_of"] = scraped_at
        profile["_as_of_target"] = as_of_target
        warnings.append("No end-of-day history available — price is taken from "
                        "the company page and may not be a settled close.")
    else:
        profile["_price_source"] = "unavailable"
        profile["_as_of_target"] = as_of_target
        warnings.append("Share price could not be retrieved.")

    # Day change is close-over-close between the last two completed sessions.
    if len(price_history) >= 2:
        prev = price_history[-2]["close"]
        last = price_history[-1]["close"]
        if prev:
            profile["change_pct"] = round((last - prev) / prev * 100, 2)
            profile["_change_basis"] = (f"{price_history[-1]['date']} close vs "
                                        f"{price_history[-2]['date']} close")
    ...
    # ---- v4.1: About block -----------------------------------------------
    # Built from the company page we ALREADY downloaded, so this costs no
    # extra network call. allow_fetch mirrors deep_pdf: the 100-company
    # ranking scan stays light, while an individual Analyze click (and the
    # /api/about endpoint) is free to consult StockAnalysis and the issuer's
    # own website for anything PSX does not publish.
    about_info: Dict = {}
    try:
        import about as _about
        about_info = _about.get_about(symbol, session=session,
                                      psx_html=page_html,
                                      allow_fetch=bool(deep_pdf))
    except Exception as exc:  # noqa: BLE001
        print(f"  [about] skipped for {symbol}: {exc}")

    return {
        "symbol": symbol,
        "profile": profile,
        "about": about_info,              # v4.1: sourced company description
        "financials": financials,
        "price_history": price_history,
        # v4.0 — the session every figure in this payload describes
        "as_of": utils.as_of_iso(),
        "as_of_close_date": (price_history[-1]["date"] if price_history else None),
        "data_stale": bool(profile.get("_price_stale")),
        "reports": reports[:6],
        "warnings": warnings,
