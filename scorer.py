...
def m_capital_adequacy(fin, banking, sa, ctx):
    ...
    if car is None:
        # v3.6 — the dedicated ladder (PSX page → StockAnalysis statistics →
        # StockAnalysis ratios → official filed reports) already ran; if we
        # are here, none of the sources publishes it *yet*. Never estimated.
        return _na("Not yet published on PSX or StockAnalysis — fills "
                   "automatically from the bank's official filed annual "
                   "report once the background deep store collects it",
                   "CAR not found on any live source",
