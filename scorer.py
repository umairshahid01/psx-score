...
def m_capital_adequacy(fin, banking, sa, ctx):
    """
    v3.6.1 — 'Capital Strength'. NA is not an option for banks:
      1) the reported Capital Adequacy Ratio when ANY source publishes it
         (PSX tables → PSX page → StockAnalysis statistics/ratios → filed
         annual reports — see scraper.fill_capital_adequacy), else
      2) Equity ÷ Total Assets — the bank's real capital cushion, computed
         from its own filed balance sheet (never estimated, always linked).
    """
    ...
    if car is not None:
        ...
        return (sub, car, _fmt_pct(car),
                f"Reported capital ratio (FY{yr}); SBP CAR floor 11.5% — "
                "exact source & link below", lbl, u)

    # ---- guaranteed fallback: Equity ÷ Total Assets from the SAME filed
    #      record (same statement ⇒ same scale), newest year first ----------
    for rec in reversed(fin):
        eq, ta, y = rec.get("total_equity"), rec.get("total_assets"), rec.get("year")
        if not eq or not ta or ta <= 0:
            continue
        ratio = eq / ta * 100.0
        if not (1.0 <= ratio <= 40.0):        # scale-mismatch guard, not a guess
            continue
        sub = higher_better(ratio, 5.0, 10.0)
        lbl, u = _rec_src(fin, "total_equity", ctx, f"FY{y} PSX financials")
        return (sub, ratio, _fmt_pct(ratio),
                f"Equity ÷ total assets (FY{y}) — the bank's capital cushion, "
                "computed from its filed balance sheet because the regulator "
                "CAR is not yet published on PSX or StockAnalysis; healthy "
                "PSX banks run ~5–10%",
                lbl, u)

    return _na("Neither the reported CAR nor the filed equity/assets pair "
               "could be read from any live source this run — re-run in a "
               "little while (the background deep store keeps collecting the "
               "bank's official filed reports)",
               "No capital figure found on any live source",
               ctx["urls"].get("sa_ratios") or ctx["urls"].get("psx"))
...
METRIC_FUNCS = {
    ...
    "capital_adequacy": ("Capital Strength",       m_capital_adequacy),
}
...
def score_company(scrape: Dict) -> Dict:
    ...
    return {
        ...
        "about":        scrape.get("about") or {},      # v4.1: sourced company
                                                        # description + products.
                                                        # Same passthrough trap as
                                                        # material_info below —
                                                        # the dashboard reads
                                                        # d.about from THIS payload.
