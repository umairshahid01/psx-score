...
## v4.0 — Ranked landing page, twin top/avoid lists & the deep-read Announcements verdict

- **Completely redesigned landing page.** The app now opens on the rankings, not a
  search box: **Top 10 Recommended Stocks** (KSE-100 ∪ KMI-30 pool) side by side with
  **10 Stocks to Stay Away From** — the worst of the worst of the **KSE-100 only** —
  both ranked by the same blended fundamental + technical engines, each card expandable
  into fundamentals bullets, technicals bullets, a plain-language verdict and (for top
  picks) the 🟢🟢🛑🎯🎯 trade-zone plan. A gradient **“In Depth Analysis — Individual
  Stock”** button gates the deep-dive workbench.
- **`recommend.py` — parallel monthly ranking engine.** Scans the whole candidate pool
  on **8 worker threads** (no more per-symbol sleeps), caches per month, and — the big
  speed win — **serves the last cached list instantly** while a fresh scan runs in the
  background, so the landing page is usable in seconds on every launch after the first.
  The build also starts **before** the deep-data prewarm at startup.
- **Individual-stock workbench.** Fresh search console with a live-expanding dropdown;
  the index tabs (All listed / KSE-100 / …) are **gone** — search always covers every
  listed company. Fundamental and Technical buttons sit below the search bar and open
  their (visually refreshed) views.
- **Announcements, deep-read edition.** Every recent filing document is now opened and
  read **in parallel** (board packs, results, material information, dividend notices),
  vague titles are re-classified from the document body, and the section shows exactly
  **one concrete 2–3 sentence verdict with real stats embedded** plus **one link** to
  the stock’s PSX announcements page. Insider/sponsor **buying is reported but carries
  zero weight** in the verdict. No more filing lists, no more link clutter.
- **RSI hover read-out** (exact RSI value + date + zone at the cursor, marker dot on
  the trend line) and **fully consistent Fundamental/Technical buttons** (shared depth,
  shine, press animation — only the colours differ).
- New footer promise: *a compass, not a captain* — the tool sharpens financial
  awareness; it is never investment advice.
