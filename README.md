...
## v3.6 — Wyckoff market cycle, real catalysts & the Capital-Adequacy ladder
...
- **Wyckoff Market Cycle analysis** in the Technical Analysis (Prediction) view.
  ...
  showing "NOW", plain-English story cards (yes, including the monkey
  story), and honest notes ("the bigger the base, the higher in the space").
...
- **NEW `catalyst.py` + Catalyst section** in the Technical Analysis view.
  The tool now pulls the company's **actual announcements from the PSX Data
  Portal** (company page first, portal-wide announcements page as fallback),
  classifies each one (expansion / plant operational / joint venture /
  contract won / demerger / insider buying / rights / dividend / red flags /
  routine), and shows every item **with a clickable link to the real
  filing**. Nothing is assumed or invented: if PSX has no growth
  announcement, the section shows a single line — *"No catalyst to
  jump-start a new rally — the chart is running on momentum and mood
  alone"* — plus a link so you can verify on PSX yourself; if the scan
  fails, it says so honestly. Routine filings are tucked behind a toggle.
- **Final verdict now weighs three factors:** Chart Health **45%** +
  Business Health **35%** + **Catalyst Fuel 20%** (a fourth score box in the
  banner). Wyckoff phase feeds the chart-health evidence (breakout +2.5,
  accumulation +1, advancing +1.5, distribution −2, decline −2.5), and both
  factors appear in "Weighing the Evidence".
- **Capital Adequacy is never silently N/A for banks anymore.** A dedicated
  4-tier ladder hunts the *reported* number: PSX statement tables → PSX
  company-page ratio cards & free text → StockAnalysis statistics page
  (Total Capital / Tier-1 / CET-1, honestly labelled) → StockAnalysis
  ratios page (per-year) → and finally the bank's official filed annual
  report via the background deep store. **Every tier records the exact
  source label and a clickable URL** shown in the metric explainer. If no
  source publishes it yet, the metric says exactly that and links you to
  the pages it checked — accurate or absent, never estimated.
