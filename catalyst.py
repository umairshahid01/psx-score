...
def _ocr_pdf_pages(buf, max_pages: int) -> str:
    ...
    no text layer). Uses pypdfium2 to rasterise + tesseract to read. Purely
    best-effort: returns '' if the OCR stack isn't installed."""
    try:
        import shutil
        import pytesseract
        import pypdfium2 as pdfium
        if not (pytesseract.get_tesseract_version() or shutil.which("tesseract")):
            return ""
    except Exception:  # noqa: BLE001
        return ""
    try:
        ...
        for i in range(min(len(pdf), max_pages)):
            try:
                ...
                out.append(pytesseract.image_to_string(bmp.to_pil()) or "")
            except Exception:  # noqa: BLE001
                continue
        pdf.close()
        return "\n".join(out)
