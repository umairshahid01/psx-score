...
def _get_rapidocr():
    """Pure-pip OCR engine (rapidocr-onnxruntime). Unlike pytesseract it
    needs NO separately-installed program, so it works out of the box on the
    machines PSX 4.0.bat sets up. Built once and shared."""
        ...
        for i in range(min(len(pdf), max_pages)):
            try:
                ...
                if tess is not None:
                    ...
                    import numpy as _np
                    res, _ = rapid(_np.array(pil))
                    out.append(" ".join(r[1] for r in (res or [])))
            except Exception:  # noqa: BLE001
                continue
        pdf.close()
        return "\n".join(out)
