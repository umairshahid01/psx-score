...
def _rows_from_announcements_page(symbol: str, session) -> List[Dict]:
    ...
    items: List[Dict] = []
    try:
        html = utils.fetch(config.PSX_ANNOUNCEMENTS_URL, session=session)
    ...
    for row in soup.find_all("tr"):
        ...
        for a in row.find_all("a", href=True):
            ...
            if not href or href.startswith(("#", "javascript")):
                continue
            full = urljoin(config.PSX_BASE, href)
            lab = ("Open filing (PDF)" if ".pdf" in full.lower()
                   or "/download/" in full else "Open on PSX")
            if full not in [u["url"] for u in urls]:
                urls.append({"label": lab, "url": full})
...
def fetch_catalysts(symbol: str, session=None, page_html: str = None) -> Dict:
    ...
    company_url = config.PSX_COMPANY_URL.format(symbol=symbol)
    out: Dict = {
        "checked": False, "error": None,
        "as_of": utils.now_iso(),
        "window_months": config.CATALYST_MONTHS,
        "source_url": company_url,
        "announcements_url": config.PSX_ANNOUNCEMENTS_URL,
    ...
    try:
        ...
        cutoff = (datetime.utcnow()
                  - timedelta(days=30.5 * config.CATALYST_MONTHS)).strftime("%Y-%m-%d")
        seen = set()
        items: List[Dict] = []
        for it in raw:
        ...
        items = items[:config.CATALYST_MAX_ITEMS]
        out["items"] = items
        ...
        if out["negatives"]:
            ...
            out["summary"] = (f"{n} real growth announcements on PSX in the last "
                              f"{config.CATALYST_MONTHS} months — the rally has "
            ...
            out["summary"] = ("No fresh catalyst on the tape — no new spark to "
                              "jump-start a rally, so the chart is running on "
