...
set "FILES=config.py utils.py psx_data.py scraper.py scorer.py predictor.py deepdata.py catalyst.py about.py exporter.py recommend.py analyzer.py dashboard.html requirements.txt"
set "DL_OK=1"
...
REM  OCR engine for scanned "Material Information" letters. Installed in its
REM  OWN step so a hiccup here can never block the core app. --no-cache-dir
REM  works around a pip wheel-cache path bug on Windows Store Python that
REM  breaks building antlr4 (a sub-dependency).
%PY% -m pip show rapidocr >nul 2>&1
if !errorlevel! NEQ 0 (
  echo   [*] Installing the OCR engine for scanned PDF letters...
  %PY% -m pip install --quiet --no-cache-dir rapidocr
  if !errorlevel! NEQ 0 (
    %PY% -m pip install --quiet --user --no-cache-dir rapidocr
  )
  %PY% -m pip show rapidocr >nul 2>&1
  if !errorlevel! NEQ 0 (
    echo   [!] Optional OCR engine could not be installed - the app still works,
    echo       but scanned Material-Information letters will show as UNREAD
    echo       unless Tesseract OCR is installed separately.
  ) else (
    echo   [ok] OCR engine ready.
  )
) else (
  echo   [ok] OCR engine ready.
)
