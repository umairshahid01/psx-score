...
def predict(scraped: Dict, fundamental: Dict) -> Dict:
    ...
    # ---- v3.6: real catalysts from PSX announcements ----------------------
    cat = scraped.get("announcements") or {}
    cat_checked = bool(cat.get("checked"))
    cat_score = float(cat.get("score", 50)) if cat_checked else 50.0
    if cat_checked:
        n_cat = len(cat.get("catalysts") or [])
        if cat.get("negatives"):
            why_bear.append("a red-flag announcement was filed on PSX — read it "
                            "before trusting the chart")
        elif n_cat >= 1:
            why_bull.append(f"{n_cat} real growth announcement(s) filed on PSX in "
                            f"the last {cat.get('window_months', 12)} months — the "
                            "move has fundamental fuel")
        else:
            why_bear.append("no fresh catalyst on the PSX tape — nothing new to "
                            "jump-start a rally")

    # catalyst is a real third factor now: chart 45% + business 35% + fuel 20%
    combined = tech_score * 0.45 + fund_score * 0.35 + cat_score * 0.20
    ...
    out.update({
        ...
        "wyckoff": wy,
        "catalyst": cat if cat_checked else {"checked": False,
                                             "error": cat.get("error"),
