...
_TOP_N = int(_CFG.get("top_n", 10))
_AVOID_N = int(_CFG.get("avoid_n", 10))
_WORKERS = max(1, int(_CFG.get("workers", 8)))
...
def _candidate_pool() -> List[str]:
    """KSE-100 (+KMI30 extras). Returns (pool, kse100_set) via attribute."""
    import psx_data
    uni = psx_data.get_universe()
    idx = uni.get("indices") or {}
    kse100 = list(idx.get(_POOL_INDEX) or [])
    pool: List[str] = list(kse100)
    for s in (idx.get(_EXTRA_INDEX) or []):
        if s not in pool:
            pool.append(s)
    ...
    pool = [s for s in pool if keep(s)][:_MAX_SYMBOLS]
    _candidate_pool.kse100 = {s for s in kse100 if keep(s)}
    return pool
...
def _build(month: str) -> None:
    """Parallel scan of the whole pool → top-N picks + worst-N avoid list."""
    global _building
    from concurrent.futures import ThreadPoolExecutor, as_completed
    results: List[Dict] = []
    try:
        pool = _candidate_pool()
        kse100 = getattr(_candidate_pool, "kse100", set(pool))
        with _lock:
        ...
        # TOP — rank by blended score; prefer non-downtrends on ties
        ...
        # AVOID — worst of the worst, strictly KSE-100 members, no overlap
        top_syms = {p["symbol"] for p in picks}
        weakest = [r for r in results
                   if r["symbol"] in kse100 and r["symbol"] not in top_syms]
        weakest.sort(key=lambda r: (r["scores"]["combined"],
                                    r.get("structure") != "downtrend"))
        avoid = []
        for rank, a in enumerate(weakest[:_AVOID_N], start=1):
            a = dict(a)
            a["rank"] = rank
...
def _latest_any_cache() -> Optional[Dict]:
    ...
    try:
        pat = os.path.join(utils.CACHE_DIR, f"{_CACHE_PREFIX}_*.json")
