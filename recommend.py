...
# v4.0.2 — after a FAILED scan (network trouble), wait before auto-retrying
# so the /api polls don't hammer-restart a doomed build every 3 seconds.
_last_fail_at = 0.0
_FAIL_COOLDOWN_S = float(_CFG.get("fail_cooldown_s", 45))
# a scan must yield at least this many usable companies to be cached —
# fewer means the NETWORK failed, not the market
_MIN_USABLE = int(_CFG.get("min_usable_results", 20))
...
def _load_cached(month: str) -> Optional[Dict]:
    ...
    try:
        ...
        with open(path, "r", encoding="utf-8") as fh:
            data = json.load(fh)
        # v4.0.2 GUARD — a cached list with no usable candidates is a
        # poisoned artefact of a failed scan (flaky network, empty index
        # fetch). Serving it deadlocks the dashboard for up to 7 days:
        # the server says "fresh cache, no rebuild needed" while the client
        # waits forever for candidates. Treat it as absent so a real scan
        # starts immediately.
...
def _evaluate(symbol: str) -> Optional[Dict]:
    ...
    try:
        ...
        if pred.get("error"):
            return None
        # v4.0 INSTANT-ANALYSIS — this exact work IS the individual analysis.
        # Prime the analyze/predict caches (memory + disk) so a Fundamental
        # or Technical click on any KSE-100 company answers instantly, with
        # real Material Information already inside.
        try:
            pred_payload = dict(pred)
            pred_payload["fundamental"] = {
            ...
            _an.prime_caches(symbol, fund, pred_payload)
        except Exception:  # noqa: BLE001
            pass
...
def _park_prewarm() -> None:
    """Tell deepdata a high-priority job is using the bandwidth — its polite
    prewarm loop then sleeps (90s grace, refreshed per candidate). This keeps
    the ranking scan at FULL network speed even on weekly quiet rebuilds."""
    try:
        import deepdata
        deepdata.note_user_activity()
    except Exception:  # noqa: BLE001
        pass
...
def _build(month: str) -> None:
    ...
    v4.0.2 — a scan that yields too few usable companies is a NETWORK
    failure and is NEVER cached; caching it would deadlock the dashboard
    behind a 'fresh' empty list for up to 7 days."""
    global _building, _last_fail_at
    ...
    try:
        ...
        if not pool:
            print("[recommend] KSE-100 index came back EMPTY — not caching; "
                  f"will retry in {int(_FAIL_COOLDOWN_S)}s")
        ...
        with ThreadPoolExecutor(max_workers=_WORKERS,
            ...
            for fut in as_completed(futs):
                ...
                _park_prewarm()          # keep the prewarm asleep all scan
                with _lock:
        ...
        min_usable = min(_MIN_USABLE, max(1, len(pool) // 3))
        if len(results) < min_usable:
            print(f"[recommend] scan yielded only {len(results)}/{len(pool)} "
                  "usable companies — that's a network problem, NOT a market "
                  f"truth. Not caching; will retry in {int(_FAIL_COOLDOWN_S)}s")
            _last_fail_at = time.time()
            return
        ...
        print(f"[recommend] scan complete — {len(results)}/{len(pool)} usable, "
              "lists cached")
    finally:
        with _lock:
            _building = False
...
def _latest_any_cache() -> Optional[Dict]:
    """Newest USABLE recommendations_*.json on disk, regardless of month.
    Files without candidates (failed scans) are skipped."""
    try:
...
def start_build(force: bool = False) -> bool:
    ...
    if cached and not force and cached.get("_age_days", 0) <= _REBUILD_DAYS:
        return False
    # v4.0.2 — a scan just failed on network grounds; brief cooldown so the
    # dashboard's polls don't hammer-restart it every few seconds
    if not force and (time.time() - _last_fail_at) < _FAIL_COOLDOWN_S:
        return False
    with _lock:
        if _building:
            return False
...
def lists_ready() -> bool:
    """True once USABLE ranked lists exist on disk (this month's, or any
    stale month when serve_stale is on). The deep-prewarm gate waits on
    this — unlike is_building(), it has no not-yet-started race."""
    if _load_cached(_month_key()) is not None:
        return True
    return bool(_SERVE_STALE and _latest_any_cache() is not None)


_prime_started = False


def prime_missing() -> None:
    """v4.0 INSTANT-ANALYSIS backstop. A launch that serves a fresh cached
    list runs NO scan — so if the analysis payloads that scan would have
    primed are missing (first launch after an upgrade, wiped cache files),
    individual clicks would fall back to the slow path. This sweep quietly
    evaluates exactly the missing candidates in the background; _evaluate
    primes both caches as it goes. On a normal steady-state relaunch every
    payload already exists and this returns immediately."""
    ...
    def _run():
        try:
            if _building:                       # a scan is priming everything
                return
            cached = _load_cached(_month_key()) or (
                _latest_any_cache() if _SERVE_STALE else None)
            if not cached:
                return                          # no lists yet — a scan will run
            import analyzer
            max_age = analyzer._STALE_MAX_H * 3600
            need = []
            for c in cached.get("candidates", []):
                sym = c.get("symbol")
                if not sym:
                    continue
                have = analyzer._pay_load("analysis", sym)
                if have is None or have[0] > max_age:
                    need.append(sym)
            if not need:
                return
            print(f"[recommend] priming {len(need)} missing analyses in the "
                  "background — clicks on already-primed stocks stay instant")
            from concurrent.futures import ThreadPoolExecutor, as_completed
            with ThreadPoolExecutor(max_workers=_WORKERS,
                                    thread_name_prefix="psx-prime") as ex:
                futs = [ex.submit(_evaluate, s) for s in need]
                for f in as_completed(futs):
                    _park_prewarm()             # keep the prewarm asleep
                    try:
                        f.result()
                    except Exception:  # noqa: BLE001
                        pass
            print("[recommend] background priming complete — every listed "
                  "company now answers instantly")
        except Exception:  # noqa: BLE001
            pass

    threading.Thread(target=_run, daemon=True, name="psx-prime").start()
